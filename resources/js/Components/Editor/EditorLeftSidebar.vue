    ...
    >
        ...
        <template v-if="leftTab === 'elements'">
            ...
            >
                <div class="relative">
                    <i
                        class="fa-solid fa-magnifying-glass absolute left-2.5 top-2.5 text-slate-400 text-xs"
                    ></i>
                    <input
                        :value="elementSearch"
                        @input="
                            $emit('update:element-search', $event.target.value)
                        "
            ...
            <div class="flex-1 overflow-y-auto p-2.5 space-y-3">
                <div v-for="cat in filteredCategories" :key="cat.name">
                    ...
                    >
                        ...
                        >
                        <i
                            class="fa-solid fa-chevron-down text-[10px] transition-transform duration-150"
                            :class="
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <i
                                    :class="el.icon + ' text-sm leading-none'"
                                ></i>
                            </div>
                            <span
                ...
                >
                    ...
                    >
                        <div
                            ...
                            :class="dark ? 'bg-slate-600' : 'bg-slate-200'"
                        ></div>
                        <div
                        ...
                        <div
                            ...
                            :class="dark ? 'bg-slate-700' : 'bg-slate-100'"
                        ></div>
                    </div>
                    ...
                    <div class="flex flex-col gap-1">
                        <button
                            @click.stop="$emit('duplicate-page', pi)"
                        ...
                        <button
                            v-if="pages.length > 1"
                            @click.stop="$emit('remove-page', pi)"
                            class="p-0.5 rounded transition-colors"
                            title="Delete"
                            :class="
                                dark
        ...
        <template v-if="leftTab === 'settings'">
            <div class="flex-1 overflow-y-auto p-3 space-y-4">
                ...
                <div>
                    ...
                    <select
                        :value="rs.page_size"
                        @change="emitRs('page_size', $event.target.value)"
                        :class="inputCls"
                    >
                        <option
                    ...
                    <div class="grid grid-cols-2 gap-1.5">
                        ...
                        >
                            <i
                                :class="
                                    o === 'portrait'
                                        ? 'fa-solid fa-file text-base mb-1'
                                        : 'fa-solid fa-file fa-rotate-90 text-base mb-1'
                                "
                            ></i>
                            {{ o }}
                        </button>
                ...
                <div>
                    ...
                    <select
                        :value="rs.font_family"
                        @change="emitRs('font_family', $event.target.value)"
                        :class="inputCls"
                    >
                        <option
                            v-for="f in FONTS"
                ...
                <div>
                    <label
                        ...
                        :class="dark ? 'text-slate-500' : 'text-slate-400'"
                    >
                        <i class="fa-solid fa-text-height mr-1"></i>Base Font
                        Size (px)
                    </label>
                    <input
                        type="number"
                        :value="rs.font_size"
                        @change="emitRs('font_size', +$event.target.value)"
                        :class="inputCls"
                        min="8"
                        max="32"
                        step="1"
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-1.5">
                                    <input
                                        ...
                                        :value="rs[cp.key]"
                                        @input="
                                            emitRs(cp.key, $event.target.value)
                                        "
                ...
                <div>
                    ...
                    <input
                        type="number"
                        :value="rs.margin"
                        @change="emitRs('margin', +$event.target.value)"
                        :class="inputCls"
                        min="0"
                        max="120"
                        step="4"
                ...
                <div>
                    <label
                        ...
                        :class="dark ? 'text-slate-500' : 'text-slate-400'"
                    >
                        <i class="fa-solid fa-vector-square mr-1"></i>Canvas
                        Border Radius (px)
                    </label>
                    <input
                        type="number"
                        :value="rs.page_radius"
                        @change="emitRs('page_radius', +$event.target.value)"
                        :class="inputCls"
                        min="0"
                        max="32"
                    />
                ...
                >
                    <div class="flex items-center justify-between">
                        ...
                        >
                            <i
                                class="fa-solid fa-rectangle-ad mr-1 text-slate-400"
                            ></i
                            >Header
                        </label>
                        <ToggleSwitch
                            :value="rs.show_header"
                            @toggle="emitRs('show_header', !rs.show_header)"
                        />
                    </div>
                    <div v-if="rs.show_header" class="space-y-2">
                        <input
                            type="text"
                            :value="rs.header_text"
                            @input="emitRs('header_text', $event.target.value)"
                            :class="inputCls"
                        ...
                        <div class="flex items-center gap-2">
                            <input
                                type="color"
                                :value="rs.header_color"
                                @input="
                                    emitRs('header_color', $event.target.value)
                                "
                                class="w-7 h-7 rounded-lg border cursor-pointer p-0.5 flex-shrink-0"
                                :class="
                                    dark
                ...
                >
                    <div class="flex items-center justify-between">
                        ...
                        >
                            <i
                                class="fa-solid fa-rectangle-ad fa-flip-vertical mr-1 text-slate-400"
                            ></i
                            >Footer
                        </label>
                        <ToggleSwitch
                            :value="rs.show_footer"
                            @toggle="emitRs('show_footer', !rs.show_footer)"
                        />
                    </div>
                    <div v-if="rs.show_footer" class="space-y-2">
                        <input
                            type="text"
                            :value="rs.footer_left"
                            @input="emitRs('footer_left', $event.target.value)"
                            :class="inputCls"
                            placeholder="Footer left…"
                        />
                        <input
                            type="text"
                            :value="rs.footer_right"
                            @input="emitRs('footer_right', $event.target.value)"
                            :class="inputCls"
                        ...
                        <div class="flex items-center gap-2">
                            <input
                                type="color"
                                :value="rs.footer_color"
                                @input="
                                    emitRs('footer_color', $event.target.value)
                                "
                                class="w-7 h-7 rounded-lg border cursor-pointer p-0.5 flex-shrink-0"
                                :class="
                                    dark
                ...
                <div class="flex items-center justify-between">
                    ...
                    >
                        <i class="fa-solid fa-hashtag mr-1 text-slate-400"></i
                        >Page Numbers
                    </label>
                    <ToggleSwitch
                        :value="rs.show_page_numbers"
                        @toggle="
                            emitRs('show_page_numbers', !rs.show_page_numbers)
                        "
                    />
                </div>
                ...
                <div>
                    ...
                    <input
                        type="text"
                        :value="rs.watermark"
                        @input="emitRs('watermark', $event.target.value)"
                        :class="inputCls"
                ...
                <div>
                    ...
                    <input
                        type="url"
                        :value="rs.bg_image"
                        @input="emitRs('bg_image', $event.target.value)"
                        :class="inputCls"
                        placeholder="https://…"
                    />
                </div>
                ...
                <div>
                    <label
                        ...
                        :class="dark ? 'text-slate-500' : 'text-slate-400'"
                    >
                        <i class="fa-solid fa-border-all mr-1"></i>Grid Size
                        (px)
                    </label>
                    <input
                        type="number"
                        :value="gridSize"
                        @change="
                            $emit('update:grid-size', +$event.target.value)
                        "
                        :class="inputCls"
                        min="5"
                        max="60"
                ...
                <div>
                    <label
                        class="prop-label mb-1.5 block"
                        :class="dark ? 'text-slate-500' : 'text-slate-400'"
                    >
                        <i class="fa-solid fa-toggle-on mr-1"></i>Display
                        Options
                    </label>
                    <div class="space-y-0">
                        <div
                        ...
                        >
                            ...
                            >
                            <ToggleSwitch
                                :value="getToggle(t.key)"
                                @toggle="setToggle(t.key)"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    >
                        <i
                            class="fa-solid fa-align-right mr-1 text-slate-400"
                        ></i
                        >RTL Layout
                    </label>
                    <ToggleSwitch
                        :value="rs.rtl"
                        @toggle="emitRs('rtl', !rs.rtl)"
                    />
                </div>
            </div>
        </template>
