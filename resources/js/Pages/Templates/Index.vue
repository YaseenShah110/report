        ...
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <!-- View toggle -->
                    <div
                        class="flex bg-slate-100 dark:bg-slate-700 rounded-xl p-1"
                    >
                        <button
                            ...
                            "
                            class="px-3 py-1.5 text-xs font-medium rounded-lg transition-all capitalize"
                        >
                            {{ v }}
                        </button>
                    </div>
        ...
        <div class="py-8 bg-slate-50 dark:bg-slate-900 min-h-full">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    >
                        ...
                        <div
                            ...
                            :style="{ background: getCoverGradient(tpl) }"
                        >
                            <div
                                class="absolute inset-0"
                                v-html="getCoverHtml(tpl)"
                            />
                            <!-- Hover overlay -->
                            <div
                            ...
                            >
                                <span
                                    class="bg-white/95 backdrop-blur text-slate-800 font-semibold text-sm px-4 py-2 rounded-full shadow-lg transform scale-90 group-hover:scale-100 transition-transform"
                                >
                                    Use Template
                                </span>
                            </div>
                            ...
                            >
                                <span
                                    class="px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-400 text-amber-900"
                                >
                                    {{ tpl.badge }}
                                </span>
                            </div>
                        ...
                        <div class="p-4">
                            ...
                            <p
                                class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2"
                            >
                                {{ tpl.description }}
                            </p>
                            <div
                            ...
                            >
                                <span
                                    ...
                                    :key="tag"
                                    class="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded text-[10px] font-medium"
                                    >{{ tag }}</span
                                >
                            </div>
                            <button
                                @click.stop="openModal(tpl)"
                    ...
                    >
                        <div
                            class="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-700 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900/40 flex items-center justify-center transition-colors"
                        >
                            <svg
                                class="w-6 h-6 text-slate-400 dark:text-slate-500 group-hover:text-indigo-500 transition-colors"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                            >
                        ...
                        <div class="text-center">
                            <p
                                class="font-semibold text-slate-700 dark:text-slate-300 text-sm"
                            >
                                Blank Canvas
                            </p>
                            <p
                ...
                <div v-else class="space-y-3">
                    ...
                    >
                        <div
                            class="w-16 h-20 rounded-xl overflow-hidden shrink-0"
                            :style="{ background: getCoverGradient(tpl) }"
                        >
                            <div
                                class="w-full h-full"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    v-if="tpl.badge"
                                    class="px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400"
                                >
                                    {{ tpl.badge }}
                                </span>
                            </div>
                            ...
                            <div class="flex gap-1.5 mt-2">
                                <span
                                    v-for="tag in tpl.tags || []"
                                    :key="tag"
                                    class="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded text-xs"
                                    >{{ tag }}</span
                                >
                            </div>
                        </div>
                        <button
                            @click.stop="openModal(tpl)"
                            class="px-5 py-2 bg-slate-900 dark:bg-indigo-600 hover:bg-indigo-600 dark:hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors shrink-0"
                        >
                            Use
                        </button>
                    </div>
                    <div
                        @click="openModal(null)"
                        class="flex items-center gap-5 bg-white dark:bg-slate-800 rounded-xl px-5 py-4 border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-indigo-400 transition-all cursor-pointer"
                    >
                        <div
                            class="w-16 h-20 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center"
                        >
                            <svg
        ...
        <Teleport to="body">
            ...
            >
                ...
                >
                    <!-- Modal header -->
                    <div
                        class="px-6 py-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between"
                    >
                        <div>
                            <h3
                    ...
                    <div class="px-6 py-5 space-y-5">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                >
                                <select
                                    v-model="reportSettings.page_size"
                                    class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                >
                            ...
                            <div>
                                ...
                                >
                                <select
                                    v-model="reportSettings.orientation"
                                    class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                >
                            ...
                            <div>
                                ...
                                <div class="flex gap-2">
                                    <input
                                        type="color"
                                        v-model="reportSettings.primary_color"
                                        class="w-10 h-[38px] rounded-lg border border-slate-200 dark:border-slate-600 cursor-pointer p-0.5 bg-white dark:bg-slate-700"
                                    />
                                    <input
                                        type="text"
                                        v-model="reportSettings.primary_color"
                                        class="flex-1 px-3 py-2 text-xs font-mono border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    />
                                </div>
                            </div>
                            <div>
                                ...
                                >
                                <select
                                    v-model="reportSettings.font_family"
                                    class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                >
                        ...
                        <div>
                            <button
                                ...
                                class="flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 font-medium hover:text-indigo-700 transition-colors"
                            >
                                <svg
                            ...
                            >
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <input
                                            type="color"
                                            v-model="
                                                reportSettings.background_color
                                            "
                                            class="w-9 h-9 rounded-lg border border-slate-200 dark:border-slate-600 cursor-pointer p-0.5 bg-white dark:bg-slate-700"
                                        />
                                        <input
                                            type="text"
                                            v-model="
                                                reportSettings.background_color
                                            "
                                            class="flex-1 px-2 py-1.5 text-xs font-mono border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-400"
                                        />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <input
                                        type="number"
                                        v-model.number="reportSettings.margin"
                                        class="w-full px-2 py-1.5 text-sm border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-400"
                                        min="0"
                                        max="120"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        v-model="reportSettings.footer_left"
                                        placeholder="Company Name"
                                        class="w-full px-2 py-1.5 text-xs border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-400"
                                    />
                                </div>
                                <div>
                                    <label
                                    ...
                                    <input
                                        type="text"
                                        v-model="reportSettings.footer_right"
                                ...
                                >
                                    <label
                                        ...
                                        >Show Page Numbers</label
                                    >
                                    <button
                                        @click="
                                            reportSettings.show_page_numbers =
                                                !reportSettings.show_page_numbers
                                        "
                                        :class="
                                            reportSettings.show_page_numbers
                    ...
                    <!-- Modal footer -->
                    <div
                        class="px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex gap-3"
                    >
                        <button
                            @click="showModal = false"
                            class="flex-1 py-2.5 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                        >
                            Cancel
                        </button>
                        <button
                            @click="createReport"
                            :disabled="!reportTitle.trim() || isCreating"
...
const getCoverHtml = (tpl) => {
    ...
    return (
        covers[tpl.slug] ||
        `<div class="absolute inset-0 flex items-center justify-center"><p class="text-white font-bold text-lg opacity-60">${tpl.name}</p></div>`
    );
};
