  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          <h2 class="text-xl sm:text-2xl font-bold tracking-tight bg-gradient-to-r from-violet-500 via-fuchsia-500 to-pink-500 bg-clip-text text-transparent">
            My Tasks
          </h2>
          <p class="text-xs sm:text-sm text-slate-400 mt-0.5 font-light tracking-wide">Manage your assigned tasks and track progress</p>
        </div>
        <div class="flex items-center gap-2 sm:gap-3">
          <!-- View Toggle -->
          <div class="flex items-center gap-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-1">
            <button
    ...
    <div class="py-5 sm:py-8 px-3 sm:px-4 lg:px-6 space-y-5 sm:space-y-7">
      ...
      <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 sm:p-4 shadow-sm">
        <div class="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          <button @click="applyFilters" class="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-xs sm:text-sm font-semibold transition-all shadow-sm shadow-violet-200 dark:shadow-violet-900/30">
            Apply
          </button>
      ...
      <div v-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5">
        <TransitionGroup name="task-card" appear>
          ...
          >
            <!-- priority stripe -->
            <div :class="`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${priorityGradient(task.priority)}`"></div>
            ...
            <div class="p-4 sm:p-5">
              ...
              <div class="flex items-start justify-between gap-2 mb-3">
                <div class="flex items-center gap-2 min-w-0">
                  ...
                  <h3 class="font-semibold text-slate-900 dark:text-white text-sm leading-tight line-clamp-2">{{ task.title }}</h3>
                </div>
                <span :class="`shrink-0 px-2 py-0.5 text-[10px] font-bold rounded-full uppercase tracking-wide ${statusBadge(task.status)}`">
              ...
              <div class="flex items-center justify-between mb-4">
                <span :class="`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${priorityBadge(task.priority)}`">
                  {{ task.priority }}
                </span>
                <div class="flex items-center gap-1.5 text-[11px]" :class="isOverdue(task.due_date) && task.status !== 'completed' ? 'text-red-500 font-semibold' : 'text-slate-400'">
                  <i class="fa-regular fa-calendar text-[10px]"></i>
                  <span>{{ task.due_date ? formatDate(task.due_date) : 'No due date' }}</span>
                  <span v-if="isOverdue(task.due_date) && task.status !== 'completed'" class="text-[9px] bg-red-100 dark:bg-red-900/30 text-red-600 px-1 rounded">LATE</span>
                </div>
              </div>
              ...
              <div class="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-700">
                ...
                <div class="flex items-center gap-1">
                  ...
                  <Link
                    v-if="task.report"
                    :href="route('reports.edit', task.report.slug)"
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30">
                ...
                <th class="text-left px-4 sm:px-6 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider hidden sm:table-cell">Due Date</th>
                <th class="text-left px-4 sm:px-6 py-3 text-[10px] font-semibold text-slate-400 uppercase tracking-wider hidden md:table-cell">Progress</th>
                ...
                >
                  <td class="px-4 sm:px-6 py-3.5">
                    <div class="flex items-start gap-2.5">
                      ...
                      <div>
                        <p class="font-semibold text-xs text-slate-900 dark:text-white line-clamp-1">{{ task.title }}</p>
                        <p class="text-[11px] text-slate-400 line-clamp-1 mt-0.5">{{ task.description }}</p>
                      </div>
                    </div>
                  </td>
                  <td class="px-4 sm:px-6 py-3.5">
                    <span :class="`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${priorityBadge(task.priority)}`">{{ task.priority }}</span>
                  </td>
                  <td class="px-4 sm:px-6 py-3.5">
                    <select
                      :value="task.status === 'overdue' ? 'pending' : task.status"
                  ...
                  <td class="px-4 sm:px-6 py-3.5 hidden sm:table-cell">
                    <span :class="['text-[11px]', isOverdue(task.due_date) && task.status !== 'completed' ? 'text-red-500 font-bold' : 'text-slate-500']">
                  ...
                  <td class="px-4 sm:px-6 py-3.5 hidden md:table-cell">
                    <div class="flex items-center gap-2">
                      <div class="w-20 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div class="h-full rounded-full transition-all duration-500" :class="progressColor(task.status)" :style="{ width: `${taskProgress(task)}%` }"></div>
                      </div>
                  ...
                  <td class="px-4 sm:px-6 py-3.5 text-right">
                    <div class="flex items-center justify-end gap-1">
                      <button @click="openTaskDetails(task)" class="p-1.5 rounded-lg hover:bg-violet-50 dark:hover:bg-violet-900/30 text-slate-400 hover:text-violet-500 transition-colors">
                        <i class="fa-regular fa-eye text-xs"></i>
                      </button>
                      <Link v-if="task.report" :href="route('reports.edit', task.report.slug)" class="p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 text-slate-400 hover:text-blue-500 transition-colors">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5">
        <div
          ...
          :class="`rounded-2xl border p-3 sm:p-4 ${col.bg}`"
        >
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
          ...
          <div class="space-y-2.5">
            <TransitionGroup name="kanban-card" appear>
              <div
                v-for="task in tasksByStatus(col.key)" :key="task.id"
              ...
              >
                <p class="font-semibold text-xs text-slate-900 dark:text-white leading-snug" :class="col.key === 'completed' ? 'line-through opacity-60' : ''">
                  {{ task.title }}
                </p>

                <div class="flex items-center justify-between mt-2.5 gap-1 flex-wrap">
                  <span :class="`text-[9px] font-bold px-1.5 py-0.5 rounded-full uppercase ${priorityBadge(task.priority)}`">{{ task.priority }}</span>

                  <span v-if="col.key === 'overdue'" class="text-[10px] text-red-500 font-semibold">
                    {{ getOverdueDays(task.due_date) }}d overdue
                  </span>
                  <span v-else-if="col.key === 'completed'" class="text-[10px] text-emerald-500 font-medium">Done ✓</span>
                  <select
                  ...
                  <button @click="openTaskDetails(task)" class="text-[10px] text-violet-500 hover:text-violet-700 font-medium transition-colors ml-auto">
                    Details →
                  </button>
                </div>
              </div>
      ...
      <div v-if="!tasks.data?.length" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <button @click="resetFilters" class="mt-4 px-5 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-sm font-semibold transition-all shadow shadow-violet-200 dark:shadow-violet-900/30">
          Clear Filters
        </button>
      </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showDetailsModal" class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6">
          ...
          <div class="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-hidden flex flex-col">
            <!-- Modal header -->
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
              ...
              <button @click="showDetailsModal = false" class="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 transition-colors">
            ...
            <div class="overflow-y-auto p-5 space-y-5 flex-1">
              ...
              <div>
                ...
                <p class="mt-1 text-xs text-slate-600 dark:text-slate-400 leading-relaxed">{{ selectedTask?.description || 'No description provided.' }}</p>
              </div>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Priority</label>
                  <div class="mt-1">
                    <span :class="`text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide ${priorityBadge(selectedTask?.priority)}`">{{ selectedTask?.priority }}</span>
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p class="mt-1 text-xs" :class="isOverdue(selectedTask?.due_date) && selectedTask?.status !== 'completed' ? 'text-red-500 font-bold' : 'text-slate-600 dark:text-slate-400'">
                ...
                <div>
                  ...
                  <p class="mt-1 text-xs text-slate-600 dark:text-slate-400">{{ formatDate(selectedTask?.created_at) }}</p>
                </div>
              </div>
              <div v-if="selectedTask?.completion_notes" class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-3">
                <label class="text-[10px] uppercase tracking-wider text-emerald-600 dark:text-emerald-400 font-semibold">Completion Notes</label>
                <p class="mt-1 text-xs text-emerald-700 dark:text-emerald-300 italic leading-relaxed">{{ selectedTask.completion_notes }}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div class="p-5 sm:p-6">
              ...
              <div class="flex gap-2.5 mt-4">
                <button @click="showNotesModal = false" class="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm font-medium transition-all">
                  Cancel
                </button>
                <button @click="submitCompletionNotes" class="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-semibold transition-all shadow shadow-emerald-200 dark:shadow-emerald-900/30">
                  Complete Task ✓
                </button>
              </div>
            </div>
...
.task-card-enter-from   { opacity: 0; transform: translateY(16px) scale(0.97); }
.task-card-leave-active { transition: all 0.2s ease; }
...
.kanban-card-enter-active { transition: all 0.3s cubic-bezier(0.34, 1.4, 0.64, 1); }
.kanban-card-enter-from   { opacity: 0; transform: translateY(10px) scale(0.96); }
.kanban-card-leave-active { transition: all 0.2s ease; }
.kanban-card-leave-to     { opacity: 0; transform: scale(0.95); }
