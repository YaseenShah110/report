<template>
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between gap-2 h-full px-1" :class="dark ? 'text-slate-100' : 'text-slate-800'">
        ...
        <div class="flex items-center gap-2 flex-1 min-w-0">
          <button @click="router.get(route('reports.index'))"
            ...
            <i class="fa-solid fa-chevron-left text-xs"></i> Reports
          </button>
          <span :class="dark ? 'text-slate-700' : 'text-slate-300'">/</span>
          <input v-model="form.title" @blur="autoSave"
          ...
          <div class="relative" ref="statusDropRef">
            <button @click="showStatusDrop = !showStatusDrop"
              class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border transition-all"
              :class="statusStyle">
            ...
            <transition name="dropdown">
              <div v-if="showStatusDrop" class="absolute top-full mt-1 left-0 w-36 rounded-xl shadow-xl border overflow-hidden z-50"
                :class="dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'">
                <button v-for="s in ['draft','published','archived']" :key="s" @click="changeStatus(s)"
                  class="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold capitalize transition-colors"
                  :class="[form.status===s ? 'text-indigo-600' : (dark ? 'text-slate-300 hover:bg-white/10' : 'text-slate-600 hover:bg-slate-50')]">
                  <span class="w-1.5 h-1.5 rounded-full" :class="s==='published'?'bg-emerald-500':s==='draft'?'bg-amber-500':'bg-slate-400'"></span>
                  {{ s }}
                </button>
              </div>
            </transition>
        ...
        <div class="flex items-center gap-1 shrink-0">
          ...
          <!-- Grid -->
          <button @click="showGrid = !showGrid" title="Grid (G)"
            class="p-1.5 rounded-lg transition-colors"
            :class="showGrid ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600' : (dark ? 'text-slate-400 hover:text-slate-200 hover:bg-white/10' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100')">
            <i class="fa-solid fa-border-all text-xs"></i>
          </button>

          <!-- Rulers -->
          <button @click="showRulers = !showRulers" title="Rulers (R)"
            class="p-1.5 rounded-lg transition-colors"
            :class="showRulers ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600' : (dark ? 'text-slate-400 hover:text-slate-200 hover:bg-white/10' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100')">
          ...
          <button @click="saveReport"
            ...
            <i class="fa-solid fa-floppy-disk text-xs"></i><span class="hidden sm:inline">Save</span>
          </button>
          ...
          <button @click="previewReport"
            ...
            <i class="fa-solid fa-eye text-xs"></i><span class="hidden sm:inline">Preview</span>
          </button>
    ...
    <transition name="toast">
      <div v-if="toast.show"
        ...
        :class="toast.type === 'error' ? 'bg-red-600 text-white' : toast.type === 'info' ? 'bg-indigo-600 text-white' : 'bg-emerald-600 text-white'">
        <i :class="toast.type === 'error' ? 'fa-solid fa-circle-exclamation' : toast.type === 'info' ? 'fa-solid fa-circle-info' : 'fa-solid fa-circle-check'"></i>
        {{ toast.message }}
      </div>
    </transition>
  </AuthenticatedLayout>
...
import {
  ...
  onMounted, onBeforeUnmount, nextTick, provide
} from 'vue';
...
const CODE_LANGS = [
  ...
  'Bash','Java','C#','C++','Go','Rust','Ruby','YAML','Markdown','Vue','React',
];
...
const ELEMENT_CATEGORIES = [
  {
    name:'Text & Typography', icon:'fa-solid fa-font', color:'#6366f1',
    items:[
  ...
  {
    name:'Labels & Callouts', icon:'fa-solid fa-tag', color:'#8b5cf6',
    items:[
  ...
  {
    name:'Charts & Graphs', icon:'fa-solid fa-chart-bar', color:'#10b981',
    items:[
  ...
  {
    name:'Data & KPIs', icon:'fa-solid fa-database', color:'#f59e0b',
    items:[
  ...
  {
    name:'Media & Embeds', icon:'fa-solid fa-photo-film', color:'#ec4899',
    items:[
  ...
  {
    name:'Shapes & Lines', icon:'fa-solid fa-shapes', color:'#0ea5e9',
    items:[
  ...
  {
    name:'Cards & Components', icon:'fa-solid fa-id-card', color:'#f97316',
    items:[
  ...
  {
    name:'Report Elements', icon:'fa-solid fa-file-lines', color:'#64748b',
    items:[
...
const pages = ref(
  props.report?.content?.length
    ? JSON.parse(JSON.stringify(props.report.content))
...
const rs = reactive({
  ...
  secondary_color:   props.report?.settings?.secondary_color   || '#10b981',
  ...
  line_height:       props.report?.settings?.line_height       || 1.6,
  letter_spacing:    props.report?.settings?.letter_spacing    || 0,
  margin:            props.report?.settings?.margin            || 40,
  padding:           props.report?.settings?.padding           || 0,
  ...
  header_height:     props.report?.settings?.header_height     || 48,
  header_color:      props.report?.settings?.header_color      || '#1e293b',
  header_text_color: props.report?.settings?.header_text_color || '#ffffff',
  header_font_size:  props.report?.settings?.header_font_size  || 12,
  header_align:      props.report?.settings?.header_align      || 'left',
  // Footer
  show_footer:       props.report?.settings?.show_footer       ?? false,
  footer_left:       props.report?.settings?.footer_left       || '',
  footer_center:     props.report?.settings?.footer_center     || '',
  footer_right:      props.report?.settings?.footer_right      || '',
  footer_height:     props.report?.settings?.footer_height     || 36,
  footer_color:      props.report?.settings?.footer_color      || 'transparent',
  footer_text_color: props.report?.settings?.footer_text_color || '#94a3b8',
  footer_font_size:  props.report?.settings?.footer_font_size  || 10,
  footer_border:     props.report?.settings?.footer_border     ?? true,
  footer_border_color:props.report?.settings?.footer_border_color || '#6366f125',
  // Page numbers
  show_page_numbers: props.report?.settings?.show_page_numbers ?? false,
  page_num_position: props.report?.settings?.page_num_position || 'footer-center',
  page_num_format:   props.report?.settings?.page_num_format   || '{page}',
  page_num_font_size:props.report?.settings?.page_num_font_size|| 10,
  page_num_color:    props.report?.settings?.page_num_color    || '#94a3b8',
  page_num_start:    props.report?.settings?.page_num_start    || 1,
  // Watermark
  watermark:         props.report?.settings?.watermark         || '',
  watermark_opacity: props.report?.settings?.watermark_opacity || 8,
  watermark_font_size:props.report?.settings?.watermark_font_size||64,
  watermark_color:   props.report?.settings?.watermark_color   || '#94a3b8',
  watermark_rotate:  props.report?.settings?.watermark_rotate  || -35,
  // Background
  bg_image:          props.report?.settings?.bg_image          || '',
  bg_image_opacity:  props.report?.settings?.bg_image_opacity  || 100,
  bg_image_fit:      props.report?.settings?.bg_image_fit      || 'cover',
  // Grid/Snap
  grid_color:        props.report?.settings?.grid_color        || '#6366f1',
  // Direction
  rtl:               props.report?.settings?.rtl               ?? false,
});

const form = useForm({
  title: props.report?.title || 'Untitled Report',
...
const statusStyle = computed(() => {
  const s = form.status;
  if (dark.value) {
    return s === 'published' ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30'
         : s === 'draft'     ? 'bg-amber-500/15 text-amber-400 border-amber-500/30'
...
: form.status === 'draft'     ? 'bg-amber-500'
:                               'bg-slate-400'
);
...
const createDefaultElement = (type, x = 60, y = 60) => {
  ...
  const P = rs.primary_color || '#6366f1';
  const T = rs.text_color || '#0f172a';

  const presets = {
    heading:    { content:'Report Heading', styles:{ ...base.styles, width:460, height:64, fontSize:32, fontWeight:'700', color:T } },
    subheading: { content:'Section Title',  styles:{ ...base.styles, width:340, height:44, fontSize:20, fontWeight:'600', color:T } },
    ...
    list:       { items:['First item','Second item','Third item'], styles:{ ...base.styles, width:280, height:120, fontSize:14, color:'#334155', listStyle:'bullet' } },
    ...
    badge:      { content:'Status', styles:{ ...base.styles, width:90, height:36, backgroundColor:P+'25', color:P } },
    ...
    watermark:  { content:'CONFIDENTIAL', styles:{ ...base.styles, width:400, height:100, fontSize:56, fontWeight:'800', color:'#94a3b8', rotate:-30, opacity:20 } },
    ...
    signature:  { content:'', label:'Authorised Signature', styles:{ ...base.styles, width:260, height:80, color:'#334155' } },
    ...
    'pie-chart':     { chartData:{ labels:['Cat A','Cat B','Cat C','Cat D'], values:[35,25,22,18] }, chartTitle:'Distribution', legendPosition:'bottom', pieColors:['#6366f1','#10b981','#f59e0b','#8b5cf6'], styles:{ ...base.styles, width:320, height:300 } },
    ...
    'price-card':  { plan:'Pro Plan', price:'$29', period:'per month', features:['Feature 1','Feature 2','Feature 3'], styles:{ ...base.styles, width:220, height:280, backgroundColor:'#fff', borderColor:'#e2e8f0', borderWidth:1, borderRadius:16 } },
  };
...
// ─── isChartType ──────────────────────────────────────────────────────────────
const isChartType = (t) =>
  ['bar-chart','line-chart','pie-chart','area-chart','doughnut-chart','radar-chart'].includes(t);
...
const initChart = (el) => {
  nextTick(() => {
    ...
    const primaryColor = el.chartColor || rs.primary_color;
    ...
    const chartInstance = new Chart(canvas.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: (type === 'pie' || type === 'doughnut') ? colors
            : isArea ? primaryColor + '30' : primaryColor + 'cc',
          borderColor: (type === 'pie' || type === 'doughnut') ? colors.map(c => c + 'dd') : primaryColor,
          borderWidth: 2,
          fill: isArea,
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: el.legendPosition === 'none' ? false : (el.legendPosition || 'bottom'),
            display: el.legendPosition !== 'none',
            labels: { padding: 12, font: { size: 11 }, boxWidth: 12, color: '#475569' },
          },
          title: {
        ...
        scales: (type === 'pie' || type === 'doughnut') ? {} : type === 'radar' ? {
          r: {
            grid: { color: '#f1f5f9' },
            ticks: { font: { size: 10 }, color: '#94a3b8', backdropColor: 'transparent' },
            pointLabels: { font: { size: 11 }, color: '#475569' },
            angleLines: { color: '#f1f5f9' },
          },
        } : {
          x: { grid: { color: '#f8fafc', drawBorder: false }, ticks: { font: { size: 11 }, color: '#94a3b8' }, border: { color: '#f1f5f9' } },
          y: { grid: { color: '#f1f5f9', drawBorder: false }, ticks: { font: { size: 11 }, color: '#94a3b8' }, border: { color: '#f1f5f9' } },
        },
      },
    });
...
const updateChartData = () => {
  ...
  el.chartDatasetLabel = chartDatasetLabel.value;
  el.chartColor = chartColor.value;
  if (pieColorsInput.value) el.pieColors = pieColorsInput.value.split(',').map(s => s.trim());
  refreshChart(el);
  pushHistory();
};
...
const reinitAllCharts = () =>
  pages.value.forEach(p =>
    p.elements.forEach(el => { if (isChartType(el.type)) initChart(el); })
  );

// ─── Element operations ───────────────────────────────────────────────────────
const addElement = (type) => {
  const pi = selectedPageIndex.value ?? 0;
  const offset = pages.value[pi].elements.length * 20;
  addElementAtPosition(type, pi, 60 + offset % 200, 60 + offset % 200);
};

const addElementAtPosition = (type, pi, x, y) => {
  const el = createDefaultElement(type, x, y);
  if (!pages.value[pi]) return;
...
const selectElement = (el, pi) => {
  ...
  if (pi !== undefined) selectedPageIndex.value = pi;
  if (el && isChartType(el.type) && el.chartData) {
...
const onTableHeaderBlur = (el, col, e) => {
  ...
  if (newName !== col) {
    ...
    if (idx > -1) {
      ...
      el.data.forEach(r => { r[newName] = r[col]; delete r[col]; });
    }
  }
  pushHistory();
};
const addTableRow = (el) => {
  const r = {}; el.columns.forEach(c => r[c] = ''); el.data.push(r); pushHistory();
};
const addTableColumn = (el) => {
  const n = `Col ${el.columns.length + 1}`; el.columns.push(n); el.data.forEach(r => r[n] = ''); pushHistory();
};
const removeTableRow = (el) => { if (el.data.length > 1) { el.data.pop(); pushHistory(); } };
const removeTableColumn = (el) => {
  if (el.columns.length > 1) { const l = el.columns.pop(); el.data.forEach(r => delete r[l]); pushHistory(); }
};
...
const triggerAutoSave = () => {
  ...
  autoSaveTimer = setTimeout(saveReport, 2500);
};
...
const exportAs = (fmt) => {
  ...
  else showToast(`${fmt.toUpperCase()} export coming soon.`, 'info');
};
...
const fitToScreen = (containerWidth) => {
  ...
  zoom.value = Math.min(150, Math.max(25, Math.floor((containerWidth / canvasDimensions.value.width) * 90)));
};
...
const handleKeyDown = (e) => {
  ...
  if ((e.ctrlKey || e.metaKey) && e.key === 'd') {
    e.preventDefault();
    if (selectedElement.value) duplicateElement(selectedPageIndex.value, selectedElement.value);
  }
  ...
  if (e.key === 'Escape') { deselectAll(); contextMenu.show = false; showStatusDrop.value = false; showExportMenu.value = false; }
...
const handleOutsideClick = (e) => {
  ...
  if (statusDropRef.value && !statusDropRef.value.contains(e.target)) showStatusDrop.value = false;
...
onMounted(() => {
  ...
  dark.value = localStorage.getItem('rb-dark') === '1' ||
    (!localStorage.getItem('rb-dark') && window.matchMedia('(prefers-color-scheme: dark)').matches);

  // Auto-save every 30 seconds
  autoSaveInterval = setInterval(saveReport, 30000);
});
