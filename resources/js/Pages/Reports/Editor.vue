<template>
    <AuthenticatedLayout>
        <template #header>
            ...
            >
                ...
                <div class="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    >
                        <i
                            class="fa-solid fa-chevron-left w-3.5 h-3.5 text-xs"
                        ></i>
                        Reports
                    </button>
        ...
        <!-- MAIN LAYOUT -->
        <div
            class="flex overflow-hidden transition-colors duration-200"
...
const createDefaultElement = (type, x = 60, y = 60) => {
    ...
    const presets = {
        ...
        checklist: {
            ...
            styles: { ...base.styles, width: 300, height: 100, fontSize: 13 },
        },
        link: {
            content: "Click here",
        ...
        timeline: {
            items: [
                { label: "Milestone 1", date: "2024 Q1", desc: "Kickoff" },
                { label: "Milestone 2", date: "2024 Q3", desc: "Launch" },
            ],
...
const removeTableColumn = (el) => {
    if (el.columns.length > 1) {
        const l = el.columns.pop();
        el.data.forEach((r) => delete r[l]);
        pushHistory();
    }
};
...
const alignElement = (dir) => {
    ...
    const el = selectedElement.value,
        pw = canvasDimensions.value.width,
...
const fitToScreen = (containerWidth) => {
    ...
    zoom.value = Math.min(
        ...
        Math.max(
            25,
            Math.floor((containerWidth / canvasDimensions.value.width) * 100),
        ),
    );
};
...
Add more editable items like modern charts ,graphs,list,tables,carts,  whose values,styles fully editableand changeable and smore then 50 using fontawsome icons and when the element is drag from left side then their properties are not showing on right sidebar , all  the element must be fully editable ,on left sidebar the page layer is must also shown with respective page name and when the page is selected then the element of that page is showing on canvas and also the properties of that element is showing on right sidebar and also add functionality to duplicate,delete the page and also add functionality to move the element from one page to another page and also add functionality to lock the element so that it cannot be moved or edited and also add functionality to align the element to left, right, center, top, bottom and also add functionality to bring the element forward or send it backward in layer order and also add functionality to fit the report to screen and also add functionality to preview the report in new tab and also add functionality to export the report in different formats like pdf, excel, csv etc and also add functionality to share the report with other users and also add functionality to set permissions for the report and also add functionality to track the changes made to the report and also add functionality to revert back to previous versions of the report and also add functionality to collaborate with other users in real time on the same report and and also their must be all require functional settings related to report like footer settings, header settings, page number settings, date and time settings and also add functionality to add custom css to the report and also add functionality to add custom js to the report and also add functionality to add custom fonts to the report and also add functionality to change the theme of the report and also add functionality to change the layout of the report and also add functionality to change the background of the report and also add functionality to change the color scheme of the report and also add functionality to change the typography of the report and also add functionality to change the spacing of the report and also add functionality to change the alignment of the report and also add functionality to change the border of the report and also add functionality to change the shadow of the report and also add functionality to change the opacity of the report and also add functionality to change the visibility of the report and also add functionality to change the z-index of the report and also add functionality to change the transform of the report and also add functionality to change the transition of the report and also add functionality to change the animation of the report and also add functionality to change the interactivity of the report and also add functionality to change the responsiveness of the report and also add functionality to change the accessibility of the report and also make sure that all these features are working properly without any bugs in code. make the report more advance with all element and their properties which must be fully editable -->
...
keep optimize and responsive design in mind while adding new features and components. and also make sure that all the features are working properly and there are no bugs in the code.
 also add functionality to dashboard for publish,darft,delete,duplicate and also add functionality to share the report with other users and also add functionality to export the report in different formats like pdf, excel, csv etc and also add functionality to set permissions for the report and also add functionality to track the changes made to the report and also add functionality to revert back to previous versions of the report and also add functionality to collaborate with other users in real time on the same report and also show the request for user to accept the request and care fully handle the sharing repoort with other user and also show the users to whom the report is accesable and also add functionality to add or remove the permission from user  -->
