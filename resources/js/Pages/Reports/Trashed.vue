            ...
            >
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-slate-500 mt-0.5 sm:mt-1">
                        Reports that have been soft-deleted
                    </p>
                </div>
                <Link
                ...
                >
                    <i class="fa-solid fa-arrow-left text-xs"></i> Back to
                    Reports
                </Link>
            </div>
        </template>
        ...
        <div class="py-6 sm:py-8 px-3 sm:px-4 lg:px-6 max-w-7xl mx-auto">
            ...
            >
                <div class="relative max-w-md">
                    <i
                        class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm"
                    ></i>
                    <input
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <span class="text-[10px] sm:text-xs text-slate-500">
                                <i class="fa-regular fa-clock mr-1"></i>Deleted
                            ...
                            ></span>
                            <span class="text-[10px] sm:text-xs text-slate-500"
                                >{{ report.total_pages || 1 }} pages</span
                            >
                        </div>
                    </div>
            ...
            <div
                v-else
                class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 py-12 sm:py-16 text-center"
            >
                <div
                    class="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center mx-auto mb-3 sm:mb-4"
                >
                    <i
                        class="fa-solid fa-trash-can text-2xl sm:text-3xl text-slate-400"
                    ></i>
                </div>
                <h3
                    class="text-base sm:text-lg font-semibold text-slate-900 dark:text-white mb-1 sm:mb-2"
                >
                    No trashed reports
                </h3>
                <p
...
const forceDeleteReport = () => {
    router.delete(
        route("reports.force-delete", deleteModal.value.report.slug),
        {
            preserveState: true,
            onSuccess: () => {
